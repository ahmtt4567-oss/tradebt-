...
export default function App() {
  ...
  return <main className={`appShell tab-${workspaceTab} view-${workspaceView}`}>
    <header>
      <div className="brand"><span className="logo">X</span><div><b>PROTREBOT ELITE X</b><small>V25 · LIVE GUARD</small></div></div>
    ...
    <Suspense fallback={<section className="moduleLazyLoading">Profesyonel çalışma alanı yükleniyor…</section>}>
      ...
      <BinanceDemo active={workspaceView === 'v20-demo'} symbol={symbol} analysis={analysis} chart={workspaceView === 'v20-demo' ? <Chart symbol={symbol} interval={interval} horizon={v8Horizon} notional={v8Notional} onAnalysis={setAnalysis} onFutureLab={setFutureLab} onLivePrice={setLivePrice} onStream={setStreamLive}/> : null}/>
    </Suspense>
    <section className={`paperPilotRibbon${panelVisibility('dashboard')}`}>
      ...
      <div className="pilotActions">
        <button className="demoOpen" disabled={paperBusy || (paper?.positions.length ?? 0) >= 3} onClick={openDemoNow}><b>＋ ŞİMDİ 50 USDT DEMO AÇ</b><small>{symbol.replace('USDT','/USDT')} · canlı fiyat · TP1/TP2/TP3</small></button>
    ...
    <section className={`bottom${panelVisibility('dashboard')}`}>
      ...
      <div className="panel controls"><h3><Bot/> V20 PAPER KONTROLLERİ</h3><button className="demoOpenCompact" disabled={paperBusy || (paper?.positions.length ?? 0) >= 3} onClick={openDemoNow}>＋ 50 USDT DEMO AÇ</button><button className={paperBot?.enabled ? 'paperStop' : 'paperStart'} disabled={paperBusy} onClick={togglePaperBot}><Power/> {paperBot?.enabled ? 'PAPER BOTU DURDUR' : 'PAPER BOTU BAŞLAT'}</button><button className={paperBot?.training_mode ? 'trainingActive' : 'trainingStrict'} disabled={paperBusy} onClick={togglePaperTraining}>{paperBot?.training_mode ? `PROFİL: ${paperBot?.profile === 'TEMKINLI' ? 'TEMKİNLİ' : paperBot?.profile === 'HIZLI' ? 'HIZLI' : 'DENGELİ'}` : '🛡 SIKI DOĞRULAMA'}</button><button className={paper?.shadow?.enabled ? 'shadowActive' : 'shadowIdle'} disabled={paperBusy} onClick={toggleShadow}>{paper?.shadow?.enabled ? '◌ GÖLGE AÇIK' : '◌ GÖLGE KAPALI'}</button><button className={paper?.emergency_brake?.active ? 'brakeReset' : 'emergency'} disabled={paperBusy} onClick={toggleEmergency}>{paper?.emergency_brake?.active ? '✓ ACİL FRENİ KALDIR' : '⚠ ACİL FREN'}</button><div className="botDiagnostics"><b>{paperBot?.events?.[0]?.kind ?? 'HAZIR'}</b><span>{paperBot?.events?.[0]?.message ?? paperBot?.last_action ?? paperMessage}</span>{paperBot?.last_blocker && <small>Son engel: {paperBot.last_blocker}</small>}</div><button disabled className="realDisabled">GERÇEK + TESTNET EMRİ KAPALI</button></div>
    </section>
