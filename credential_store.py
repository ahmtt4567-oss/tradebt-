...
def load_live_credentials() -> tuple[str, str]:
    ...
    Hosted deployments use Render environment secrets.  The Windows DPAPI
    vault remains available for the desktop build.  A partially configured
    environment fails closed instead of silently falling back to a different
    credential set.
    """
