...
function LivePositionChart({token,symbol,interval,plan}:{token?:string;symbol:string;interval:string;plan:Plan|null}) {
  ...
  useEffect(() => {
    ...
    const chart = createChart(ref.current,{
      ...
      rightPriceScale:{borderColor:'#3c4034'},
      timeScale:{borderColor:'#3c4034',timeVisible:true,secondsVisible:false},
...
export default function ExecutionCenter({token=''}:{token?:string}) {
  ...
  return <div className="executionV25">
    ...
    <section className="executionCard scannerCard">
      <div className="executionHead"><div><small>V25 OTOMATİK TARAMA</small><h3>Tarama, Seçim ve Gerçekleşen İşlemler</h3></div><Bot/></div>
      <div className="scannerMetrics"><span><small>TARANAN</small><b>{status.scanner.scanned_symbol_count}</b></span><span><small>ADAY</small><b>{status.scanner.candidate_count}</b></span><span><small>SEÇİLEN TOP 3</small><b>{status.scanner.selected_symbols_count}/3</b></span><span><small>AÇILAN</small><b>{status.scanner.executed_symbols_count}</b></span><span><small>SON TARAMA</small><b>{date(status.scanner.last_scan_at)}</b></span></div>
      <div className="scannerLists"><div><small>SON 100 TARAMA</small><p>{status.scanner.candidate_symbols.length ? status.scanner.candidate_symbols.join(' · ') : 'Henüz tarama yapılmadı.'}</p></div><div><small>EN İYİ 3 FIRSAT</small><p>{status.scanner.selected_symbols.length ? status.scanner.selected_symbols.join(' · ') : 'Uygun LONG/SHORT fırsatı yok.'}</p></div><div><small>EMİR AÇILAN SEMBOLLER</small><p>{status.scanner.executed_symbols.length ? status.scanner.executed_symbols.join(' · ') : 'Bu taramada işlem açılmadı.'}</p></div></div>
    </section>
