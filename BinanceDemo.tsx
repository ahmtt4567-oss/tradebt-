...
export default function BinanceDemo({active,symbol,analysis,chart}:{active:boolean;symbol:string;analysis:AnalysisPlan|null;chart?:ReactNode}) {
  ...
  return <section className="binanceDemoDeck" aria-label="Binance Futures Demo Köprüsü" data-build-marker="BUILD_COMMIT" data-build-commit={import.meta.env.VITE_BUILD_COMMIT} data-position-source="reconciled_active_positions" data-diagnostics="exchange_position_diagnostics">
    ...
    {tab === 'auto' && <section className="v21Workspace">
      ...
      <article className="v21Card v21AutoScanner"><header><Gauge/><div><small>100 USDT PERPETUAL TARAMA</small><h3>Bot Durumu ve En İyi Fırsatlar</h3></div><b>{v21?.scanner.scan_status || v21?.scanner.last_stage || 'BEKLEMEDE'}</b></header><div className="v21MetricRow"><span><small>TARAMA</small><b>{v21?.scanner.coins_scanned ?? 0} coin</b></span><span><small>UYGUN FIRSAT</small><b>{v21?.scanner.selected_count ?? v21?.scanner.eligible_count ?? 0}</b></span><span><small>AÇIK POZİSYON</small><b>{v21?.account.positions ?? 0}/{v21?.settings.max_positions ?? 3}</b></span><span><small>SON TARAMA</small><b>{stamp(v21?.scanner.last_scan_at)}</b></span><span><small>SONRAKİ TARAMA</small><b>{stamp(v21?.scanner.next_scan_at)}</b></span></div><div className="v21ScannerCandidates">{v21?.scanner.top_candidates.length ? v21.scanner.top_candidates.map(candidate => <span key={candidate.symbol}><b>{candidate.symbol}</b><em>{candidate.direction}</em><strong>{candidate.score.toFixed(2)}</strong><small>{candidate.trend} · {candidate.momentum} · güven {candidate.confidence}</small></span>) : <p>{v21?.scanner.last_error || 'İlk 100 coin taraması bekleniyor.'}</p>}</div></article>
